...
import { Plus, Edit2, Trash2, Building2, Users as UsersIcon, Upload } from 'lucide-react';
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Bâtiments</h1>
        {user?.role === 'ADMIN' && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {buildings.map((b) => (
          <div key={b.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-start justify-between mb-3">
              ...
              {user?.role === 'ADMIN' && (
                <div className="flex gap-1">
                  ...
                  <button onClick={() => handleDelete(b.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-400"><Trash2 className="w-4 h-4" /></button>
                </div>
              )}
            </div>
            ...
            {b.users?.[0] && <p className="text-xs text-gray-400">Gestionnaire: {b.users[0].name}</p>}
            {user?.role === 'ADMIN' && (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold mb-4">{editing ? 'Modifier' : 'Nouveau'} bâtiment</h2>
      ...
      {importing && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setImporting(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold mb-4">Importer des résidents</h2>
            <p className="text-sm text-gray-500 mb-4">Format: Prénom, Nom, Appartement, Téléphone, Email (colonnes A-E)</p>
            <input type="file" accept=".xlsx,.xls" onChange={handleImport} className="mb-4 text-sm" />
            <button onClick={() => setImporting(false)} className="w-full py-2.5 border border-gray-300 rounded-lg text-sm">Fermer</button>
          </div>
        </div>
      )}
    </div>
