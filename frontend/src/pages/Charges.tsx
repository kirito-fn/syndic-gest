        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              ...
              {user?.role === 'ADMIN' && <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase">Actions</th>}
            </tr>
          </thead>
          <tbody>
            {charges.map((c) => (
              <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50">
                ...
                {user?.role === 'ADMIN' && <td className="px-4 py-3 text-sm text-gray-500">{c.building?.name}</td>}
                <td className="px-4 py-3 text-sm text-gray-700">{monthNames[c.month - 1]} {c.year}</td>
                <td className="px-4 py-3 text-sm font-medium text-gray-800">{c.amount.toFixed(2)} €</td>
                {user?.role === 'ADMIN' && (
                  <td className="px-4 py-3 text-right">
                    ...
                    <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-400"><Trash2 className="w-4 h-4" /></button>
                  </td>
                )}
              </tr>
            ))}
            {charges.length === 0 && <tr><td colSpan={5} className="text-center py-8 text-gray-400 text-sm">Aucune charge</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold mb-4">{editing ? 'Modifier' : 'Nouvelle'} charge</h2>
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                ...
                <select value={form.year} onChange={(e) => setForm({ ...form, year: Number(e.target.value) })} className="border border-gray-300 rounded-lg px-3 py-2 text-sm">
                  {[2024, 2025, 2026].map((y) => <option key={y} value={y}>{y}</option>)}
                </select>
              </div>
            </div>
