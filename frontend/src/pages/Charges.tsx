...
export default function Charges() {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm(t('charges.deleteConfirm'))) return;
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">{t('charges.title')}</h1>
        {user?.role === 'ADMIN' && (
          <button onClick={() => { setEditing(null); setForm({ buildingId: 0, amount: 0, month: new Date().getMonth() + 1, year: new Date().getFullYear(), description: '' }); setShowModal(true); }}
            ...
            <Plus className="w-4 h-4" /> {t('charges.new')}
          </button>
        )}
      </div>
          ...
          <tbody>
            {charges.map((c) => (
              <tr key={c.id} className="border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                ...
                <td className="px-4 py-3 text-sm font-medium text-gray-800 dark:text-gray-200">{c.amount.toFixed(2)} {currency}</td>
                {user?.role === 'ADMIN' && (
            ...
            {charges.length === 0 && <tr><td colSpan={5} className="text-center py-8 text-gray-400 dark:text-gray-500 text-sm">{t('common.noCharge')}</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">{editing ? t('charges.edit') : t('charges.new')}</h2>
            <div className="space-y-3">
              <input placeholder={t('charges.description')} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
              ...
              {user?.role === 'ADMIN' && (
                <select value={form.buildingId} onChange={(e) => setForm({ ...form, buildingId: Number(e.target.value) })}
                  ...
                  <option value={0}>Sélectionner un {t('common.buildings')}</option>
                  {buildings.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
                </select>
              )}
