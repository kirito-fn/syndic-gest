...
import { Search, Filter, CheckCircle, XCircle, Clock, RotateCcw, MessageSquare, Mail, Phone, AlertTriangle, DollarSign } from 'lucide-react';
...
export default function Payments() {
  ...
  const statusBadge = (status: string) => {
    ...
    const labels: Record<string, string> = { PAID: t('payments.statusPaid'), PENDING: t('payments.statusPending'), UNPAID: t('payments.statusUnpaid') };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">{t('payments.title')}</h1>
        <button onClick={() => setShowDeclare(true)}
          className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">
          <CheckCircle className="w-4 h-4" /> {t('payments.declare')}
        </button>
      </div>
          ...
          <tbody>
            {payments.map((p) => (
              <tr key={p.id} className="border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                ...
                <td className="px-4 py-3 text-sm font-medium text-gray-800 dark:text-gray-200">{p.amount.toFixed(2)} {currency}</td>
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => setShowLogs(showLogs === p.id ? null : p.id)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 dark:text-gray-500" title={t('payments.logs')}>
                      <Clock className="w-3.5 h-3.5" />
                    </button>
                    {p.status === 'PENDING' && user?.role === 'ADMIN' && (
                      <button onClick={() => handleVerify(p.id)} className="p-1.5 rounded-lg hover:bg-green-50 dark:hover:bg-green-900/30 text-green-600 dark:text-green-400" title={t('payments.verify')}>
                        <CheckCircle className="w-3.5 h-3.5" />
                      </button>
                    )}
                    {p.status === 'PAID' && user?.role === 'ADMIN' && (
                      <button onClick={() => handleUnverify(p.id)} className="p-1.5 rounded-lg hover:bg-yellow-50 dark:hover:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400" title={t('payments.unverify')}>
                        <RotateCcw className="w-3.5 h-3.5" />
                      </button>
                    )}
                    {p.status === 'PAID' && (
                      <button onClick={() => sendConfirmation(p.id)} className="p-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 text-blue-600 dark:text-blue-400" title={t('payments.confirmation')}>
                        <MessageSquare className="w-3.5 h-3.5" />
                      </button>
                    )}
                    {(p.status === 'PENDING' || p.status === 'PAID') && user?.role === 'ADMIN' && (
                      <button onClick={() => handleReset(p.id, p.status)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400" title={t('payments.reset')}>
                        <XCircle className="w-3.5 h-3.5" />
                      </button>
                    )}
                    {user?.role === 'ADMIN' && (
                      <button onClick={() => { setStatusTarget({ id: p.id, newStatus: p.status === 'PAID' ? 'UNPAID' : 'PAID' }); setShowStatusModal(true); }}
                        className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 dark:text-gray-500" title={t('payments.changeStatus')}>
                        <Filter className="w-3.5 h-3.5" />
                      </button>
                    )}
                    {p.status === 'PENDING' && (
                      <button onClick={() => handleNoPayment(p.id)} className="p-1.5 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/30 text-orange-600 dark:text-orange-400" title={t('payments.noPayment')}>
                        <AlertTriangle className="w-3.5 h-3.5" />
                      </button>
                    )}
                    {p.status === 'PENDING' && (
                      <button onClick={() => handleMarkUnpaid(p.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 text-red-600 dark:text-red-400" title={t('payments.markUnpaid')}>
                        <XCircle className="w-3.5 h-3.5" />
                      </button>
                    )}
                  </div>
            ...
            {payments.length === 0 && <tr><td colSpan={8} className="text-center py-8 text-gray-400 dark:text-gray-500 text-sm">{t('common.noPayment')}</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {showDeclare && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowDeclare(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-xl p-6 w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">{t('payments.declareTitle')}</h2>
            <div className="space-y-3">
              <select value={declareForm.residentId} onChange={(e) => setDeclareForm({ ...declareForm, residentId: Number(e.target.value) })}
                ...
                <option value={0}>{t('payments.selectResident')}</option>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowDeclare(false)} className="flex-1 py-2.5 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-gray-800">{t('common.cancel')}</button>
              <button onClick={handleDeclare} className="flex-1 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">{t('payments.declare')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showStatusModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowStatusModal(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">{t('payments.changeStatus')}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">{t('payments.newStatus')}: <strong className="text-gray-800 dark:text-gray-200">{statusTarget?.newStatus === 'PAID' ? t('payments.statusPaid') : t('payments.statusUnpaid')}</strong></p>
            <textarea value={statusReason} onChange={(e) => setStatusReason(e.target.value)} placeholder={t('payments.reason')}
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowStatusModal(false)} className="flex-1 py-2.5 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-lg text-sm hover:bg-gray-50 dark:hover:bg-gray-800">{t('common.cancel')}</button>
              <button onClick={handleStatusChange} className="flex-1 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">{t('common.confirm')}</button>
            </div>
          </div>
        </div>
      )}
