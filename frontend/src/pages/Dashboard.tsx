...
export default function Dashboard() {
  ...
  if (!stats) return <div className="flex items-center justify-center h-64 text-gray-500 dark:text-gray-400">Chargement...</div>;

  const cards = [
    { label: 'Résidents', value: stats.totalResidents, icon: Users, color: 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' },
    { label: 'Payés', value: stats.paidCount, icon: CheckCircle, color: 'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400' },
    { label: 'En attente', value: stats.pendingCount, icon: Clock, color: 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400' },
    { label: 'Impayés', value: stats.unpaidCount, icon: XCircle, color: 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400' },
    { label: 'Encaissé', value: `${stats.totalCollected.toFixed(2)} €`, icon: Euro, color: 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' },
    { label: 'Charges', value: `${stats.totalCharges.toFixed(2)} €`, icon: TrendingUp, color: 'bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400' },
  ];
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">Résumé financier - {monthNames[month - 1]} {year}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Encaissements mensuels ({year})</h2>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={stats.monthlyData}>
              ...
              <Tooltip
                ...
                formatter={(value: number) => `${value.toFixed(2)} €`} labelFormatter={(m) => monthNames[Number(m) - 1]} />
              <Bar dataKey="total" fill="#6366f1" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Progression par bâtiment</h2>
