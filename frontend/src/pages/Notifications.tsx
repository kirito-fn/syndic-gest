...
export default function Notifications() {
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        {user?.role === 'ADMIN' && (
          <select value={filterBuilding} onChange={(e) => setFilterBuilding(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              ...
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Envoyé par</th>
          ...
          <tbody>
            {logs.map((log) => (
              <tr key={log.id} className="border-b border-gray-50 hover:bg-gray-50">
                <td className="px-4 py-3 text-sm text-gray-500">{new Date(log.createdAt).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</td>
                <td className="px-4 py-3">
                  <span className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full w-fit ${log.type === 'EMAIL' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>
                    {log.type === 'EMAIL' ? <Mail className="w-3 h-3" /> : <MessageSquare className="w-3 h-3" />}
                    {log.type}
                  </span>
                </td>
                <td className="px-4 py-3 text-sm text-gray-700">{log.resident?.firstName} {log.resident?.lastName}</td>
                ...
                <td className="px-4 py-3"><span className="text-xs font-medium text-green-700 bg-green-100 px-2 py-1 rounded-full">{log.status}</span></td>
              </tr>
            ))}
            {logs.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-gray-400 text-sm">Aucune notification</td></tr>}
          </tbody>
        </table>
      </div>
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-lg mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={sendForm.selectAll} onChange={(e) => setSendForm({ ...sendForm, selectAll: e.target.checked, residentIds: [] })} />
                Sélectionner tous les résidents
              </label>
              <div className="max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-2 space-y-1">
                {filteredResidents.map((r) => (
                  <label key={r.id} className={`flex items-center gap-2 p-1.5 rounded text-sm cursor-pointer ${sendForm.residentIds.includes(r.id) ? 'bg-indigo-50' : 'hover:bg-gray-50'}`}>
              ...
              <textarea value={sendForm.message} onChange={(e) => setSendForm({ ...sendForm, message: e.target.value })} placeholder="Votre message..."
                className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500 min-h-[100px]" />
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowSend(false)} className="flex-1 py-2.5 border border-gray-300 rounded-lg text-sm">Annuler</button>
              <button onClick={handleSend} className="flex-1 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">Envoyer</button>
            </div>
          </div>
        </div>
      )}
