  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Résidents</h1>
      ...
      <div className="flex gap-3 mb-4">
        ...
        {user?.role === 'ADMIN' && (
          <select value={buildingFilter} onChange={(e) => setBuildingFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-sm">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              ...
              <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Prénom</th>
          ...
          <tbody>
            {residents.map((r) => (
              <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50">
                <td className="px-4 py-3 text-sm font-medium text-gray-800">{r.apartment}</td>
                <td className="px-4 py-3 text-sm text-gray-700">{r.lastName}</td>
                <td className="px-4 py-3 text-sm text-gray-700">{r.firstName}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <a href={`mailto:${r.email}`} className="p-1 text-gray-400 hover:text-indigo-600"><Mail className="w-4 h-4" /></a>
                    <a href={`https://wa.me/${r.phone.replace(/\s/g, '')}`} target="_blank" className="p-1 text-gray-400 hover:text-green-600"><Phone className="w-4 h-4" /></a>
                  </div>
                </td>
                {user?.role === 'ADMIN' && <td className="px-4 py-3 text-sm text-gray-500">{r.building?.name}</td>}
                <td className="px-4 py-3 text-right">
                  <button onClick={() => openEdit(r)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => handleDelete(r.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-400"><Trash2 className="w-4 h-4" /></button>
                </td>
              </tr>
            ))}
            {residents.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-gray-400 text-sm">Aucun résident</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold mb-4">{editing ? 'Modifier' : 'Nouveau'} résident</h2>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 border border-gray-300 rounded-lg text-sm">Annuler</button>
